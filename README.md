...
+ 关系抽取虽然deepseek也能做得很好，但是它实在是太慢了（30秒左右才能改一个回复），阿里云百炼平台上的DeepSeek-R1-Distill-Llama-8B其实也不错（但是最近的一些例子似乎显示它很容易返回一个数字，很奇怪），但是比gpt-4o的幻觉会多一些，多语种效果可能也不够好。llama-4-maverick-17b-128e-instruct还要更好一点，但是目前qwen平台上对这个是限流的。
...
+ 目前很多代码其实是在处理返回值等逻辑，但其实工作流、Agent等新技术已经可以缓解这方面的变成成本，实现更可视化、更灵活的调用和管理。只是目前豆包仅兼容豆包和Deepseek：https://www.coze.cn/，而dify和cursor等都是国外的，只能调用OpenAI，还是有些贵。