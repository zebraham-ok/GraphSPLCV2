...
# 添加阈值自动调整函数
def find_optimal_threshold(probas, labels, target_recall=0.95):
...
if __name__ == "__main__":
    ...
    positive_file = "DataSet\\with_supply_data_embed.json"
    negative_file = "DataSet\\without_supply_data_embed.json"
    ...
    def load_data_from_json(positive_file, negative_file):
        ...
        return (
            [item["embedding"] for item in positive_data],
            [item["embedding"] for item in negative_data],
            [item["content"] for item in positive_data],
            [item["content"] for item in negative_data],
        )
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Train Loss: {train_loss:.4f} | Test Loss: {test_loss:.4f} | Recall @0.3: {test_recall:.4f}")
