...
# 定义一个函数来处理每个重复实体
def process_dup_entity(record, neo4j_host:API.neo4j_SPLC.Neo4jClient):
    try:
    ...
    for in_rel_info in all_incoming:
        if "source_id" in in_rel_info:
            neo4j_host.Crt_rel_by_id(
                ...
                end_node_id=same_name_node_id,   # 把循环时的最后一个节点作为保留的节点
...
def main():
    ...
    while True:
        ...
        with ThreadPoolExecutor(max_workers=15) as executor:  # 设置最大线程数为10
            futures = [executor.submit(process_dup_entity, record, neo4j_host) for record in dup_entity]
