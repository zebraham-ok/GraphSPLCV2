...
def ai_entity_recognition(article_title, content):
    ...
    ai_response=API.ai_ask.ask_qwen(prompt_text='''
            ...
            3. 请结合你的知识和检索的信息，返回“以上文中每一个具体的组织实体作为key，以该具体组织实体的正式中文全称作为value”的字典，称为full_cn_name_dict，作为结果的第三项。如果原文中出现的就是组织实体的正式中文全称，不变即可。
            4. 如果该段落中有代词代指具体的组织实体（如“本公司”、“该企业”等），请在pronoun_entity一栏列出，用一个字典形式说明其在原文中指代对象的正式中文全称，作为结果的第四项。
            请以json格式回答问题，格式如下所示：
            {
                "all_entities": list[str],
                "specific_entities": list[str],
                "full_cn_name_dict": dict[原文中的组织实体名: 正式中文全称],
